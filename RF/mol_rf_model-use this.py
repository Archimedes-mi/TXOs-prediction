...
def visualize_performance(metrics, target_name, save_path="results"):
    ...
    # 设置标题和轴标签
    plt.title(f'{target_name.upper()} Prediction Performance')
    plt.xlabel(f'True {target_name.upper()}')
    plt.ylabel(f'Predicted {target_name.upper()}')
...
def visualize_cv_results(results_df, target_name):
    ...
    for i, (metric, title) in enumerate(zip(metrics, titles)):
        ...
        bars = plt.bar(results_df['fold'], results_df[metric], color=colors['blue' if i==0 else 'teal' if i==1 else 'cyan'])
        ...
        for bar in bars:
            ...
            plt.text(bar.get_x() + bar.get_width()/2., height + 0.01 * max(results_df[metric]),
